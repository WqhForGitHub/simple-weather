...
class _ParamidaPay {
  constructor() {
    this.ApiCaller = {
      ...
      airhost: "https://free-api.heweather.net/s6/air",
      // get请求
      get(uri, data = {}, success = null, error = null) {
        ...
        Vue.http.get(this.host + uri, { params: data, _timeout: 5000 }).then(
          (response) => {
      ...
      // get请求获取空气质量
      getAirQuailty(uri, data = {}, success = null, error = null) {
        // this.jsonp(uri, data, success, error);
        // return;
        if (success == undefined) {
          success = null;
        ...
        Vue.http.get(this.airhost + uri, { params: data, _timeout: 5000 }).then(
          (response) => {
            let parsedBody = this.parseBody(response);
            success(parsedBody);
          },
      ...
      post(uri, data = {}, success, error) {
        ...
        // return;
        Vue.http
          .post(this.host + uri, this.formUrlencode(data), {
      ...
      put(uri, data = {}, success, error) {
        Vue.http
          .put(this.host + uri, this.formUrlencode(data), {
            headers: { "content-type": "application/x-www-form-urlencoded" },
            _timeout: 30000
          })
          .then((response) => {
      ...
      jsonp(uri, data = {}, success, error) {
        Vue.http.jsonp(this.host + uri + "?" + this.formUrlencode(data)).then(
          (response) => {
            let parsedBody = this.parseBody(response);
            success(parsedBody);
          },
